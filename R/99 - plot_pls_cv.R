...
PLSDA_CV <- function(df, method = "LOOCV",
                     compNum = 5, choice = "Q2") {
  match.arg(method, c("LOOCV", "CV"))
  ...
  plsda.cls <- caret::train(datmat, group, "pls",
                            trControl = caret::trainControl(method = method),
                            tuneLength = compNum);

  # use the classifical regression to get R2 and Q2 measure
  plsda.reg <- pls::plsr(group ~ datmat, method ='oscorespls',
                         ncomp = compNum,
                         validation = ifelse(method == 'LOOCV', "LOO", 'CV'));
  ...
  if(class(coef.mat) == "try-error") {
    ...
    if(length(unique(group)) > 2){ # add an average coef for multiple class
...
plot_PLS_CV <- function(df, method = "CV", compNum = 5, choice = "Q2") {
  match.arg(method, c("LOOCV", "CV"))
  match.arg(choice, c("Q2", "R2", "Accuracy"))

  plsda <- PLSDA_CV(df, method, compNum, choice)

  res <- plsda$fit.info;
  colnames(res) <- 1:ncol(res);
  best.num <- plsda$best.num;
  choice <- plsda$choice;

  par(mar = c(5,5,2,7)); # put legend on the right outside
  barplot(res, beside = TRUE,
          col = c("lightblue", "mistyrose","lightcyan"),
          ylim= c(0,1.05), xlab = "Number of components", ylab = "Performance")
